...
async function main() {
  ...
  const liveDir = path.join('test-results', 'live-caps-png');
  const outDir = path.join('test-results', 'local-vs-live-diff');
  ...
  for (const c of cases) {
    ...
    const livePath = path.join(liveDir, `${c.id}.png`);
    ...
    const liveImg = readPNGToImage(fs.readFileSync(livePath));
  ...
  await fs.promises.writeFile('LOCAL_VS_LIVE_COMPARISON.md', lines.join('\n'));
  console.log('Comparison complete. See LOCAL_VS_LIVE_COMPARISON.md and test-results/local-vs-live-diff/*.png');
}
